...
records = player_data.filter(['player_id','player_name']).sort_values(by='player_name').to_dict('records')
...
with st.sidebar:
    ...
    with st.form('Form1'):
        ...
        start_y, end_y = st.slider('Select a range of years', 1997, 2021, (1997, 2021))
...
if not submitted:
    ...
    with col3:
        ...
        st.metric(
            ...
            delta=round((df.shot_made_flag.mean() - df_l.shot_made_flag.mean()) * 100, 1)
            )
    ...
    with col4:
        ...
        if len(df.loc[df['value'] == 3]) > 0:
            st.metric(
                label="3PT%",
                value=round(df.loc[df['value'] == 3].shot_made_flag.mean() * 100, 1),
                delta=round(
                    (df
                     .loc[df['value'] == 3]
                     .shot_made_flag
        ...
        st.metric(
            label="xPPS*",
            value=round((df.value * df.pred_make).sum() / df.shot_made_flag.count(), 2),
            delta=round(
    ...
    with col1:
        ...
        sns.barplot(
            ...
            y='shot_zone_basic',
            palette=player_data.team_abbreviation.map({k:v[0] for (k, v) in COLORS.items()})
            )
        ...
        for i, (x, y, xx, v) in enumerate(zip(bar_w, bar_h, ex_vals, lg_vals)):
            line1 = plt.vlines(
                ...
                ymin=(i+(y/2)-0.01),
                ymax=(i-(y/2)),
                color=player_data.team_abbreviation.map({k:v[-1] for (k, v) in COLORS.items()}),
                linestyle='dotted',
                zorder=10,
                label='League Avg.'
                )
    ...
    if viz.gp.max() <= 1:
        ...
        line1 = sns.lineplot(
            ...
            color=player_data.team_abbreviation.map({k:v[0] for (k, v) in COLORS.items()}).iloc[0],
            zorder=3, label='Field Goal %'
            )
        line2 = sns.lineplot(
            x=viz.gp, y=viz.pred_make.rolling(r).mean(), linewidth=2,
            color=player_data.team_abbreviation.map({k:v[-1] for (k, v) in COLORS.items()}).iloc[0],
        ...
        ax.fill_between(
            ...
            where=viz.shot_made_flag.rolling(r).mean() > viz.pred_make.rolling(r).mean(),
            color=player_data.team_abbreviation.map({k:v[0] for (k, v) in COLORS.items()}).iloc[0],
